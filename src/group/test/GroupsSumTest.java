...
import group.sum.GroupsSum;
import group.sum.GroupsSumTaskThread;
import group.sum.GroupsSumThreadPool;
...
int[][] largeGroups = getLargeGroups(N_GROUPS, GROUP_LENGTH);
	...
	private int[][] getLargeGroups(int nGroups, int groupLength) {
		// DONE creating random two dimensional array 
		// using method generate of Stream
		Random random = new Random();
		return IntStream.range(0, nGroups)
				.mapToObj(i -> random.ints(groupLength,1,100).toArray())
		        .toArray(int[][]::new);
	}
	@Test
